...
class arquivos:
    def __init__(self, root):
        ...
        root.title('Jogo da Forca - Bravoow Tecnologia')
    ...
    def forca(self, event):
        ...
        b=str(self.nom.get().upper()[0]) #transforma em string e s� pega o primeiro
        #caracter
        for t in range(len(lista_letras)):
            if b == lista_letras[t]:
                ...
                if b not in digito:
                    digito.append(b)#s� adciona ao digito se n�o houver nele
        if b not in lista_letras:
            lista_erro.append(b)
            self.msg2['text']=lista_erro
        self.caixa.set('')
        if len(digito) == len(lista_traco):
            self.msg3['text']='Jogo Ganho! Parab�ns!'
            self.msg3['fg']='green'
        if len(lista_erro) == 10:
            self.msg3['text']='10 erros, voc� perdeu!',lista_letras
            self.msg3['fg']='red'
            self.msg.destroy()
            self.nom.destroy()
